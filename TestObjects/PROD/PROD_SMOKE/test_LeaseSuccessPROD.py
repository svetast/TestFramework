...
class TestLeaseCreatingSuccessPROD(LeasePage):
    """Test scenario:
        ...
        6.Return to start test data: Go to ADMIN and change order status to Cancelled """
    ...
    # @pytest.mark.skip
    def test_LeaseSuccessPROD_CurrentUser(self):
        driver = self.driver
        ...
        LoginPage.logIn(self, "svetast" + str(userNumber), "Ss123456")
        HomePage.submitSearch(self, 'canon')
        SearchResultsPage.clickAddToCart(self)
        driver.get(url_2)
        time.sleep(8)
        ...
        CartPage.selectPriceReviewedOptIn(self)
        CartPage.clickReviewYourLease(self)
        LeasePage.signAgreement(self, firstName='sveta', lastName=' step' + str(userNumber))
        time.sleep(35)
        self.assertIn(text1, self.driver.page_source)
        driver.get(url_2)
        time.sleep(5)
        ...
        # OrdersAdminPage.findOrdersByLastname(self, 'step8078')
    ...
    def test_LeaseSuccessPROD_Tires(self):
        ...
        #####  Return Test Data ##########
        driver.get(url_3)
        ConfiguratorAdminPage.loginAdminProd(self)
        driver.get(url_3 + '/Secure/Orders/OrderManagement/ViewOrders/Default.aspx')
